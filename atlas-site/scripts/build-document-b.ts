...
import { getOpenAIConfig, requestJsonSchema } from "./lib/openai.ts";
...
async function resolveIssuerWithOpenAI(
  ...
  const schema = {
    ...
    properties: {
      issuer: {
        ...
        properties: {
          name: { type: "string" },
          type: { type: "string", enum: ["company", "person", "unknown"] },
    ...
    required: ["issuer", "ticker", "cik", "aliases", "notes"]
  };
  const systemPrompt =
    "You are an issuer resolver for institutional finance records. Return JSON only.";
  const userInput = JSON.stringify({
    ledger: {
      title: ledger.title,
      theme: ledger.theme,
      canonicalUrl: ledger.canonicalUrl,
      entityGuess: ledger.entityGuess
    },
    meta
  });
  return requestJsonSchema<{
    ...
    aliases: string[];
    notes: string;
  }>(config, "issuer_resolution", schema, systemPrompt, userInput);
}

async function verifySourcesWithOpenAI(
  config: ReturnType<typeof getOpenAIConfig>,
  issuer: { name: string; aliases: string[]; ticker: string | null; cik: string | null },
  sources: Array<{ url: string; title: string | null; tier: "primary" | "secondary" }>
) {
  const schema = {
    type: "object",
    additionalProperties: false,
    properties: {
      ...
      reason: { type: "string" },
      issuerConfirmedBySources: { type: "boolean" },
      highConfidenceCount: { type: "number" }
    },
    required: ["verdict", "reason", "issuerConfirmedBySources", "highConfidenceCount"]
  };
  const systemPrompt =
    "You verify issuer consistency across sources. Return JSON only.";
  const userInput = JSON.stringify({ issuer, sources });
  return requestJsonSchema<{
    verdict: "pass" | "fail";
    reason: string;
    issuerConfirmedBySources: boolean;
    highConfidenceCount: number;
  }>(config, "source_consistency", schema, systemPrompt, userInput);
}

async function writeDocumentWithOpenAI(
  config: ReturnType<typeof getOpenAIConfig>,
  template: string,
  payload: {
    issuer: { name: string; type: string };
    theme: Theme;
    sources: Array<{ url: string; title: string | null; note: string }>;
    facts: string[];
  }
) {
  const schema = {
    ...
    properties: {
      ...
      agents: { type: "string" },
      structuralQuestion: { type: "string" }
    },
    required: ["opening", "historical", "mechanism", "agents", "structuralQuestion"]
  };
  const systemPrompt =
    "You write an institutional, minimally imperial document. Return JSON only with section text.";
  const userInput = JSON.stringify({ template, payload });
  return requestJsonSchema<{
    opening: string;
    historical: string;
    mechanism: string;
  ...
  }>(config, "document_sections", schema, systemPrompt, userInput);
}

function collectFactsFromSources(sources: SourceEntry[]) {
  const facts: string[] = [];
...
async function collectSourcesOpenAI(
  ledger: LedgerRecord,
  issuer: { name: string; aliases: string[]; ticker: string | null; cik: string | null },
  ...
  const sources: SourceEntry[] = [];
  const accessedAtUTC = formatUtc();
  const aliases = Array.from(new Set([issuer.name, ...issuer.aliases].filter(Boolean)));
  const tickers = issuer.ticker ? [issuer.ticker] : [];
  const issuerSignals = evaluateIssuerSignals(issuer.name, aliases, tickers);

  const canonicalMeta = await fetchMetaIfAllowed(ledger.canonicalUrl, allowedDomains);
  if (!canonicalMeta.skipped) {
    const metaText = [
      canonicalMeta.meta?.ogTitle,
      canonicalMeta.meta?.twitterTitle,
      canonicalMeta.meta?.pageTitle,
      canonicalMeta.meta?.jsonLdTitle
    ]
      .filter(Boolean)
      .join(" ");
    const canonicalSignals = metaText
      ? matchIssuerSignals(metaText, aliases, tickers)
      : { matchedAlias: null, confidence: "low" as const };
    if (canonicalSignals.confidence === "high") {
      sources.push({
        url: ledger.canonicalUrl,
        publisher: ledger.source.name,
        tier: "secondary",
  ...
  if (issuer.cik) {
    try {
      const submissionsResponse = await loadSecSubmissionsWithMeta(issuer.cik);
      const submissions = submissionsResponse.data;
      const formsPriority = ["10-Q", "10-K", "8-K", "6-K", "424B4", "S-1", "S-1/A", "F-1", "F-1/A"];
      const primaryList = pickLatestFilings(submissions, formsPriority);
      if (primaryList.length) {
        const primary = primaryList[0];
        sources.push({
          url: buildSecArchiveUrl(issuer.cik, primary.accessionNumber, primary.primaryDocument),
          publisher: "SEC EDGAR",
          tier: "primary",
          accessedAtUTC,
          note: `Form ${primary.form} filed ${primary.filingDate}.`,
      ...
      const secondary = pickLatestFilings(submissions, formsPriority);
      for (const entry of secondary) {
        if (sources.length >= 5) break;
        const url = buildSecArchiveUrl(issuer.cik, entry.accessionNumber, entry.primaryDocument);
        if (sources.some((item) => item.url === url)) continue;
        sources.push({
          url,
          publisher: "SEC EDGAR",
      ...
      if (issuer.ticker && sources.length < 5) {
        ...
        sources.push({
          ...
          accessedAtUTC,
          note: "Exchange listing page.",
          extractedIssuerSignals: {
            matchedAlias: ticker.toLowerCase(),
            confidence: "high"
          }
        });
      }
  ...
  return { sources: sources.slice(0, 5), warnings };
}
...
async function runOpenAiFlow(
  ...
  if (!ledgerPath.found || !ledgerPath.path) {
    const summary = {
      ...
      sourcesCount: 0,
      imageSelected: false,
      gates: { issuer: "fail", sources: "fail" },
  ...
  ledger.title = normalizeTitle(ledger.title);

  const allowedDomains = await loadWhitelistDomains();
  const canonicalMeta = await fetchMetaIfAllowed(ledger.canonicalUrl, allowedDomains);
  const meta = canonicalMeta.meta;

  const config = getOpenAIConfig();
  const warnings: string[] = [];
  const issuerResolution = await resolveIssuerWithOpenAI(config, ledger, meta);
  const normalizedCik = issuerResolution.cik ? issuerResolution.cik.padStart(10, "0") : null;
  const issuerGate = issuerResolution.issuer.confidence === "high" ? "pass" : "fail";
  ...
  if (issuerGate === "fail") {
    const payload: SourcesPayload = {
      ...
      recoveryNotes: [],
      issuerResolution,
      gateStatus: { issuer: "fail", sources: "fail" }
    };
    await writeSourcesPayload(sourcesPath, payload);
    await fs.writeFile(
      statusPath,
      JSON.stringify(
        {
          ...
          reason: "issuer_confidence_low",
          issuer: issuerResolution.issuer,
          ticker: issuerResolution.ticker,
          cik: normalizedCik
        },
        null,
        2
  ...
  await sleep(250);
  const sourceResult = await collectSourcesOpenAI(
    ledger,
    {
      name: issuerResolution.issuer.name,
      aliases: issuerResolution.aliases,
      ticker: issuerResolution.ticker,
      cik: normalizedCik
    },
    allowedDomains
  ...
  if (sources.length < 5) {
    const payload: SourcesPayload = {
      ...
      recoveryNotes: [],
      issuerResolution,
      gateStatus: { issuer: "pass", sources: "fail" }
    };
    await writeSourcesPayload(sourcesPath, payload);
    await fs.writeFile(
      statusPath,
    ...
    const summary = {
      ...
      imageSelected: false,
      gates: { issuer: "pass", sources: "fail" },
      warnings
    };
    process.stdout.write(`${JSON.stringify(summary, null, 2)}\n`);
    return;
  ...
  for (const source of sources) {
    const metaResult = await fetchMetaIfAllowed(source.url, allowedDomains);
    const title =
      metaResult.meta?.ogTitle ||
      metaResult.meta?.twitterTitle ||
      metaResult.meta?.jsonLdTitle ||
      metaResult.meta?.pageTitle ||
      null;
    sourcesWithTitles.push({ url: source.url, title, tier: source.tier });
    await sleep(100);
  }
  const consistency = await verifySourcesWithOpenAI(
    config,
    {
      name: issuerResolution.issuer.name,
      aliases: issuerResolution.aliases,
  ...
  const sourcesGate = consistency.verdict === "pass" ? "pass" : "fail";
  ...
  if (sourcesGate === "fail") {
    ...
    const summary = {
      ...
      imageSelected: false,
      gates: { issuer: "pass", sources: "fail" },
      warnings: [...warnings, consistency.reason]
    };
    process.stdout.write(`${JSON.stringify(summary, null, 2)}\n`);
    return;
  }

  const { image, warnings: imageWarnings } = await selectImage(sources, docsDir);
  warnings.push(...imageWarnings);
  const template = await fs.readFile(path.join(rootDir, TEMPLATE_PATH), "utf8");
  const facts = collectFactsFromSources(sources);
  const sectionData = await writeDocumentWithOpenAI(config, template, {
    issuer: issuerResolution.issuer,
    theme: ledger.theme,
    sources: sourcesWithTitles.map((source) => ({
      url: source.url,
      title: source.title,
      note: sources.find((entry) => entry.url === source.url)?.note ?? ""
    })),
    facts
  });
  const sections = {
    opening: sectionData.opening,
    historical: sectionData.historical,
    mechanism: sectionData.mechanism,
    agents: sectionData.agents,
    structuralQuestion: sectionData.structuralQuestion
  };
  ...
  const payload: SourcesPayload = {
    ...
    recoveryNotes: [],
    issuerResolution,
    gateStatus: { issuer: "pass", sources: "pass" }
  };
  await writeSourcesPayload(sourcesPath, payload);
  await fs.writeFile(
    statusPath,
  ...
  const summary = {
    ...
    gates: { issuer: "pass", sources: "pass" },
    warnings
  };
  process.stdout.write(`${JSON.stringify(summary, null, 2)}\n`);
}
...
async function main() {
  ...
  if (process.env.OPENAI_API_KEY) {
    try {
      await runOpenAiFlow(recordId, force);
      return;
    } catch (error) {
      const docsDir = path.join(process.cwd(), "docs", recordId);
      await fs.mkdir(docsDir, { recursive: true });
      const statusPath = path.join(docsDir, "status.json");
      const sourcesPath = path.join(docsDir, "sources.json");
      const payload: SourcesPayload = {
      ...
      const summary = {
        ...
        imageSelected: false,
        gates: { issuer: "fail", sources: "fail" },
        warnings: ["openai_error", (error as Error).message]
      };
      process.stdout.write(`${JSON.stringify(summary, null, 2)}\n`);
      return;
    }
